...
use ::rusoto_datapipeline::{QueryObjectsInput, DescribeObjectsOutput, DescribeObjectsError, QueryObjectsOutput, QueryObjectsError, DescribeObjectsInput};
...
fn get_pipeline_id_names(data_pipeline_client: &DataPipelineClient) -> Vec<PipelineIdName> {
    let mut all_pipelines: Vec<PipelineIdName> = vec![];
    let mut _marker = Some("".to_string());
    while _marker.is_some() {
        let list_pipelines_output = data_pipeline_client.list_pipelines(ListPipelinesInput { marker: _marker.clone() }).sync();
...
fn get_pipeline_tasks(pipeline_id: String, client: &DataPipelineClient, allowed_statuses: &Vec<PipelineTaskStatus>) -> Vec<PipelineTasks> {
    ...
    let query_objects_response: Result<QueryObjectsOutput, QueryObjectsError> = client.query_objects(query_objects_input).sync();
    ...
    let describe_objects_response: Result<DescribeObjectsOutput, DescribeObjectsError> = client.describe_objects(describe_objects_input).sync();
...
fn get_pipelines_descriptions(pipeline_ids: Vec<String>, data_pipeline_client: &DataPipelineClient) -> Vec<PipelineDescription> {
    ...
    while _start < _max {
        ...
        let describe_pipelines_output = data_pipeline_client.describe_pipelines(DescribePipelinesInput {pipeline_ids: subset_pipelines}).sync();
...
pub fn status(client: &DataPipelineClient, pipeline_name_filters: &Vec<String>, filter_operation: &str) -> Vec<Pipeline> {
    ...
    let now = Utc::now();
    let pipeline_ids = get_pipeline_id_names(&client).iter().flat_map(|pin| &pin.id).map(|c| c.clone()).collect();
    let pipelines_status: Vec<Pipeline> = get_pipelines_descriptions(pipeline_ids, &client).iter()
        ...
        .flat_map( |pipeline_desc| {
            ...
            let tasks = get_pipeline_tasks(
                pipeline_desc.pipeline_id.clone(),
                client,
                &allowed_status_query);
